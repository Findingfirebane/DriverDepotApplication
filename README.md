...
3. Update `v1/src/main/resources/database.properties`:
   Change: `jdbc.url=jdbc:mysql://localhost:3306/vehiclemanagement`
   To:     `jdbc.url=jdbc:mysql://db:3306/vehiclemanagement`
...
Then open and run: `v1/src/main/resources/sql/01-vehiclemanagementDDL.sql`

4. Run the population script to insert test data:

   `v1/src/main/resources/sql/02-dbpopulation.sql`
...
Note: If running via Docker, change `localhost` to `db` in the jdbc.url above.
...
- **Table not found** - ensure you have run both SQL scripts in the correct order (01 first, then 02)
- **Access denied to /manageVehicles** - you must be signed in as a staff user to access this page
- **Docker build failed** - ensure Docker Desktop is running and database.properties has been updated to use `db` instead of `localhost`